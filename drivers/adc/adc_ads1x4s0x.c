...
LOG_MODULE_REGISTER(ads1x4s0x, CONFIG_ADC_LOG_LEVEL);
...
static void ads1x4s0x_data_ready_handler(const struct device *dev, struct gpio_callback *gpio_cb,
					 uint32_t pins)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(pins);

	struct ads1x4s0x_data *data =
		CONTAINER_OF(gpio_cb, struct ads1x4s0x_data, callback_data_ready);

	k_sem_give(&data->data_ready_signal);
}

static int ads1x4s0x_read_register(const struct device *dev,
				   enum ads1x4s0x_register register_address, uint8_t *value)
{
...
static int ads1x4s0x_write_register(const struct device *dev,
				    enum ads1x4s0x_register register_address, uint8_t value)
{
	const struct ads1x4s0x_config *config = dev->config;
	uint8_t buffer_tx[3];
...
static int ads1x4s0x_write_multiple_registers(const struct device *dev,
					      enum ads1x4s0x_register *register_addresses,
...
static int ads1x4s0x_send_command(const struct device *dev, enum ads1x4s0x_command command)
{
	const struct ads1x4s0x_config *config = dev->config;
	uint8_t buffer_tx[1];
	const struct spi_buf tx_buf[] = {{
...
static int ads1x4s0x_channel_setup(const struct device *dev,
				   const struct adc_channel_cfg *channel_cfg)
{
	...
	int result;
	enum ads1x4s0x_register register_addresses[7];
	...
	result = ads1x4s0x_write_multiple_registers(dev, register_addresses, values,
						    ARRAY_SIZE(values));
...
{
	struct ads1x4s0x_data *data = CONTAINER_OF(ctx, struct ads1x4s0x_data, ctx);
...
static int ads1x4s0x_adc_start_read(const struct device *dev, const struct adc_sequence *sequence,
				    bool wait)
{
	int result;
...
static int ads1x4s0x_send_start_read(const struct device *dev)
{
	const struct ads1x4s0x_config *config = dev->config;
	int result;
...
static int ads1x4s0x_wait_data_ready(const struct device *dev)
{
	struct ads1x4s0x_data *data = dev->data;
...
#if CONFIG_ADC_ASYNC
static int ads1x4s0x_adc_read_async(const struct device *dev, const struct adc_sequence *sequence,
				    struct k_poll_signal *async)
{
	...
	result = ads1x4s0x_adc_start_read(dev, sequence, true);
	adc_context_release(&data->ctx, result);

	return result;
}

static int ads1x4s0x_read(const struct device *dev, const struct adc_sequence *sequence)
{
	int result;
	struct ads1x4s0x_data *data = dev->data;
...
static int ads1x4s0x_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	adc_context_lock(&data->ctx, false, NULL);
	result = ads1x4s0x_adc_start_read(dev, sequence, false);

	while (result == 0 && k_sem_take(&data->ctx.sync, K_NO_WAIT) != 0) {
		result = ads1x4s0x_adc_perform_read(dev);
	}
...
#if CONFIG_ADC_ASYNC
static void ads1x4s0x_acquisition_thread(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);
...
static int ads1x4s0x_gpio_write_config(const struct device *dev)
{
	struct ads1x4s0x_data *data = dev->data;
	enum ads1x4s0x_register register_addresses[2];
	...
	return ads1x4s0x_write_multiple_registers(dev, register_addresses, register_values,
						  ARRAY_SIZE(register_values));
}

static int ads1x4s0x_gpio_write_value(const struct device *dev)
{
	struct ads1x4s0x_data *data = dev->data;
	uint8_t gpio_dat = 0;
	...
	return ads1x4s0x_write_register(dev, ADS1X4S0X_REGISTER_GPIODAT, gpio_dat);
}
...
int ads1x4s0x_gpio_set_pin_value(const struct device *dev, uint8_t pin, bool value)
{
	...
	if ((BIT(pin) & data->gpio_enabled) == 0) {
		...
		result = ads1x4s0x_gpio_write_value(dev);
	}
...
int ads1x4s0x_gpio_port_set_masked_raw(const struct device *dev, gpio_port_pins_t mask,
...
static int ads1x4s0x_init(const struct device *dev)
{
	...
	gpio_init_callback(&data->callback_data_ready, ads1x4s0x_data_ready_handler,
	...
	k_tid_t tid = k_thread_create(&data->thread, config->stack,
				      ...
				      ads1x4s0x_acquisition_thread, (void *)dev, NULL, NULL,
				      CONFIG_ADC_ADS1X4S0X_ASYNC_THREAD_INIT_PRIO, 0, K_NO_WAIT);
...
static DEVICE_API(adc, api) = {
	.channel_setup = ads1x4s0x_channel_setup,
	.read = ads1x4s0x_read,
